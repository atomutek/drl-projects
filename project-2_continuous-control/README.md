...
This project makes use of the `Reacher` environment which contains 20 identical agents, each with its own copy of the environment.  
...
2. Place the file in the directory root and unzip (or decompress) the file.

#### Python setup
The python version used here is `3.6.5`.

The mandatory libraries can be installed by running the following command:
 `pip install -r requirements.txt`

For the visualization notebook, you need to install:
* `graphviz` software
* `torchviz` python library

### Instructions

Follow the instructions in `Continuous-Control_DDPG.ipynb` to get started with training your own agent!  
